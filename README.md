...
- **工具变更归因账本:** 开启 `autoPreCommandSnapshot` 后，插件会把每个成功或失败的高风险工具调用与其前置边界之间的 workspace delta 记录为 `toolMutations`；可用 CLI `/tm-tool-mutations <checkpoint>`、`GET /api/tool-mutations?...` 和 Dashboard 查看。它是路径级审计证据，不会猜测作者，也不替代 turn 级回滚；失败调用还会保存最多 300 字符的清洗错误摘要（不保存原始参数/完整输出），即使宿主的 `tools/result` 晚于 `turn/end` 到达，也会按 callId/执行对象关联并保留最多 5 分钟。
...
- 未配置 `shadowStoreEncryptionKeyEnv` 时，Shadow Git objects 仍是明文 at rest，`GET /api/storage` 会明确返回 `gitObjectsEncrypted: false`；配置后会在归档已就绪（或尚未产生任何对象）时返回 `gitObjectsEncrypted: true`。若检测到旧明文目录，`GET /api/capabilities` 会返回 `shadowStoreMigrationRequired: true`，此时先运行迁移再继续写入。插件不会直接改写 Git loose object/pack 字节来伪装加密。
...
| ---: | ---: | ---: | ---: |
| 100 | 2.61× | 805 / 868 | 0.062× |
| 1,000 | 0.42× | 1,406 / 1,515 | 0.090× |
| 10,000 | 0.19× | 2,935 / 3,004 | 0.139× |

这是 2026-09-19 Windows Node 22 的一次可重复合成 TypeScript 文件基准（每组 5 turns），不代表所有真实仓库；机器负载会影响绝对延迟。基准先提交一个
...
同日复测（同一脚本、5 turns）实际得到 1,000 文件 Git 平均 1,427.76ms
（P50/P95 1,395.57/1,505.04ms），10,000 文件平均 2,833.09ms
（2,809.01/2,894.54ms）；对应传统复制平均 2,661.89ms 和 12,180.30ms。
这些数字用于持续回归，不应被当作固定性能 SLA。
