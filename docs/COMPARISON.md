...
| Symlink / hard-link restore safety | Symlinks are represented explicitly; restore rejects ancestor symlinks/non-directories and hard-linked target inodes before mutation, while fallback unlinks before writing | Hermes refuses unsupported mounted/special paths; SiriLee explicitly skips symlink/hard-link targets and refuses symlink ancestors | Product-specific | Often unspecified |
...
| Workspace route inspection | `GET /api/workspace-route` and `TimeMachineClient.workspaceRoute()` expose the host cwd spelling, workspace id and honest isolation mode; the service validates existence and compares canonical realpaths, so platform aliases are safe while invalid routes fail closed | Host/UI-dependent | Host/UI-dependent | Usually absent |
